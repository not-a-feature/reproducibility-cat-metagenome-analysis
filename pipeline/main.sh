...
$WS/pipeline/filter_raw_reads.sh $ID_1
$WS/pipeline/filter_raw_reads.sh $ID_2
$WS/pipeline/filter_raw_reads.sh $ID_3
$WS/pipeline/filter_raw_reads.sh $ID_4


# Create metagenomic reference
conda activate cat-megahit-env
$WS/pipeline/megathit.sh $ID_1 $ID_2 $ID_3 $ID_4

# Filter metagenomic reference and annotate it
conda activate cat-diamond-env
$WS/pipeline/filter_cdhit_diamond_megan.sh $WS/data/assembly/final.contigs.fa

# Align preprocessed reads against metagenomic reference
conda activate cat-base-env

$WS/pipeline/bwa_metagenome.sh $ID_1
$WS/pipeline/bwa_metagenome.sh $ID_2
$WS/pipeline/bwa_metagenome.sh $ID_3
$WS/pipeline/bwa_metagenome.sh $ID_4
